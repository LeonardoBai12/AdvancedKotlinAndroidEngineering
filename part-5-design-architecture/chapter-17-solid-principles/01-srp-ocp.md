...
Martin defines the scope of SOLID precisely in *Clean Architecture* (Part III): the principles tell us how to organise functions and data structures into classes, and how those classes should be interconnected. "Uma classe é apenas um agrupamento acoplado de funções e dados" — a class is merely a coupled grouping of functions and data, and every system has such groupings whether it calls them classes or not. The principles operate at the **mid-level** — above individual lines of code, inside the modules and components that shape the system — and they aim to create software structures that:
...
[↑ Chapter Index](./README.md) · [Next: LSP & ISP →](./02-lsp-isp.md)
